...
static sm_vec3_t mag_current = {0};   /* μT, raw                  */
...
void Attitude_Update(float dt) {
    ...
    /* 磁力计: 原始值 / 4096 → μT (ICM-20948 内置 AK09916) */
    ...
    if (calib_handle.state == CALIB_ACCEL_COLLECTING &&
        ...
        if (calib_handle.state == CALIB_ACCEL_DONE) {
            ...
            Persistence_WriteCalibData(PERSISTENCE_DATA_MARKER,
    ...
    if (accel_calib.is_valid) {
        float calibrated[3];
        Calib_Accel_Apply(&accel_calib, accel_current, calibrated);
        accel_current[0] = calibrated[0];
        accel_current[1] = calibrated[1];
        accel_current[2] = calibrated[2];
    }
