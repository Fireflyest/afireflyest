 ...
 *  ├─────────────┼────────────────────────────────────────────────────────────┤
 *  │  光流        │  机体系水平地面投影速度 [velocity_x, velocity_y], m/s      │
 *  │             │  velocity_x: 沿 X_b (前方) 速度分量                        │
 *  │             │  velocity_y: 沿 Y_b (右侧) 速度分量                        │
 *  │             │  由调用方根据光流角速度 × 距地高度计算后填入                   │
 *  └─────────────┴────────────────────────────────────────────────────────────┘
 *
 * ============================================================================
 *  观测模型说明
 ...
 *     m_body = R_x(+90°) · [m_N, m_E, m_D]ᵀ = [m_N, m_D, -m_E]  ✓
 *
 *   验证 (机头朝天 90°, Pitch=+90°):
 *     R_y(+90°) = [[0, 0, -1],[0, 1, 0],[1, 0, 0]]
 *     m_body = R_y(+90°) · [m_N, m_E, m_D]ᵀ = [-m_D, m_E, m_N]  ✓
 *
 * 初始化要求:
 ...
 *
 * 陀螺仪补偿:
 *   integrated_xgyro / integrated_ygyro 记录积分期间陀螺仪角位移 (rad)，
 *   用于扣除飞行器自身旋转在光流中引起的分量。
 *
 * 局限:
 *   - 仅在纹理丰富的地面有效
 *   - 高速旋转时受旋转分量污染，需陀螺仪补偿
