...
const uint32_t bmi260_config_size = 8192;

/* ═══════════════════════════════════════════════════
 *  延时
 * ═══════════════════════════════════════════════════ */

static bmi260_delay_fn_t s_delay = NULL;
...
static bmi260_err_t read_reg(uint8_t sensor_id, uint8_t reg, uint8_t* val) {
    SPI_Sensor_On(sensor_id);
    SPI_Sensor_TransferByte(reg | 0x80);
    SPI_Sensor_TransferByte(0x00); /* dummy byte */
    *val = SPI_Sensor_TransferByte(0x00);
    SPI_Sensor_Off(sensor_id);
    return BMI260_OK;
}

static bmi260_err_t read_regs(uint8_t sensor_id, uint8_t reg, uint8_t* buf, uint8_t len) {
    SPI_Sensor_On(sensor_id);
    SPI_Sensor_TransferByte(reg | 0x80);
    SPI_Sensor_TransferByte(0x00); /* dummy byte */
...
static bmi260_err_t write_reg(uint8_t sensor_id, uint8_t reg, uint8_t val) {
    SPI_Sensor_On(sensor_id);
    SPI_Sensor_TransferByte(reg & 0x7F);
    SPI_Sensor_TransferByte(val);
    SPI_Sensor_Off(sensor_id);
    return BMI260_OK;
}
...
bmi260_err_t BMI260_WhoAmI(bmi260_dev_t* dev, uint8_t* chip_id) {
    if (!dev || !chip_id)
        return BMI260_ERR_PARAM;
    return read_reg(dev->sensor_id, BMI2_CHIP_ID_ADDR, chip_id);
}
...
static bmi260_err_t soft_reset(bmi260_dev_t* dev) {
    bmi260_err_t rslt;
    uint8_t dummy;

    rslt = write_reg(dev->sensor_id, BMI2_CMD_REG_ADDR, BMI2_SOFT_RESET_CMD);
    if (rslt != BMI260_OK)
        return rslt;
    delay_ms(2);
...
/* ═══════════════════════════════════════════════════
 *  初始化（严格按照官方流程：sec_init → soft_reset → config）
 * ═══════════════════════════════════════════════════ */
...
bmi260_err_t BMI260_ReadAccel(bmi260_dev_t* dev, bmi260_raw_xyz_t* acc) {
    if (!dev || !acc)
        return BMI260_ERR_PARAM;
    uint8_t buf[6];
    bmi260_err_t rslt = read_regs(dev->sensor_id, BMI2_ACC_X_LSB_ADDR, buf, 6);
    if (rslt != BMI260_OK)
        return rslt;
    acc->x = (int16_t)((uint16_t)buf[1] << 8 | buf[0]);
    acc->y = (int16_t)((uint16_t)buf[3] << 8 | buf[2]);
    acc->z = (int16_t)((uint16_t)buf[5] << 8 | buf[4]);
    return BMI260_OK;
}
...
bmi260_err_t BMI260_ReadGyro(bmi260_dev_t* dev, bmi260_raw_xyz_t* gyr) {
    if (!dev || !gyr)
        return BMI260_ERR_PARAM;
    uint8_t buf[6];
    bmi260_err_t rslt = read_regs(dev->sensor_id, BMI2_GYR_X_LSB_ADDR, buf, 6);
    if (rslt != BMI260_OK)
        return rslt;
    gyr->x = (int16_t)((uint16_t)buf[1] << 8 | buf[0]);
    gyr->y = (int16_t)((uint16_t)buf[3] << 8 | buf[2]);
    gyr->z = (int16_t)((uint16_t)buf[5] << 8 | buf[4]);
    return BMI260_OK;
}
...
bmi260_err_t BMI260_ReadAll(bmi260_dev_t* dev, bmi260_data_t* data) {
    if (!dev || !data)
        return BMI260_ERR_PARAM;
    uint8_t buf[14];
    bmi260_err_t rslt = read_regs(dev->sensor_id, BMI2_ACC_X_LSB_ADDR, buf, 14);
    if (rslt != BMI260_OK)
        return rslt;

    data->acc.x = (int16_t)((uint16_t)buf[1] << 8 | buf[0]);
    data->acc.y = (int16_t)((uint16_t)buf[3] << 8 | buf[2]);
    data->acc.z = (int16_t)((uint16_t)buf[5] << 8 | buf[4]);
    data->gyr.x = (int16_t)((uint16_t)buf[7] << 8 | buf[6]);
    data->gyr.y = (int16_t)((uint16_t)buf[9] << 8 | buf[8]);
    data->gyr.z = (int16_t)((uint16_t)buf[11] << 8 | buf[10]);
    data->temp_raw = (int16_t)((uint16_t)buf[13] << 8 | buf[12]);
